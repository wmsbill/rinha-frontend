(() => {
  ...
  var require_clarinet = __commonJS({
    "node_modules/clarinet/clarinet.js"(exports) {
      (function(clarinet3) {
        ...
        clarinet3.parser = function(opt) {
          return new CParser(opt);
        };
        clarinet3.CParser = CParser;
        clarinet3.CStream = CStream;
        clarinet3.createStream = createStream;
        clarinet3.MAX_BUFFER_LENGTH = 64 * 1024;
        clarinet3.DEBUG = env.CDEBUG === "debug";
        clarinet3.INFO = env.CDEBUG === "debug" || env.CDEBUG === "info";
        clarinet3.EVENTS = [
          "value",
          "string",
          "key",
        ...
        var buffers = {
          textNode: void 0,
          numberNode: ""
        }, streamWraps = clarinet3.EVENTS.filter(function(ev) {
        ...
        for (var s_ in clarinet3.STATE)
          clarinet3.STATE[clarinet3.STATE[s_]] = s_;
        S = clarinet3.STATE;
        const Char = {
          tab: 9,
          // \t
        ...
        function checkBufferLength(parser) {
          var maxAllowed = Math.max(clarinet3.MAX_BUFFER_LENGTH, 10), maxActual = 0;
          ...
          parser.bufferCheckPosition = clarinet3.MAX_BUFFER_LENGTH - maxActual + parser.position;
        }
        ...
        function CParser(opt) {
          ...
          var parser = this;
          clearBuffers(parser);
          parser.bufferCheckPosition = clarinet3.MAX_BUFFER_LENGTH;
        ...
        function write(chunk) {
          ...
          while (c) {
            ...
            if (!c)
              break;
            if (clarinet3.DEBUG)
              console.log(i, c, clarinet3.STATE[parser.state]);
            if (!lockIncrements) {
              parser.position++;
              if (c === Char.lineFeed) {
                parser.line++;
  ...
  function customJSONParser(text, pageSize = 250, onPage2) {
  ...
  function parseFile(file) {
    ...
    processJSON(file, 250, onPage).then((lines) => {
